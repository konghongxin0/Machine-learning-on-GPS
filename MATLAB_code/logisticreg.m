...
%-----logistic regression regression progress----
trainrate=[0];
testrate=[0];
trainr=zeros(1,6000);
testr=0;
...
sely(1,1:2)=averagey(1,1:2);
  ...
  for t=1:1:50
      ...
      %tic
        while(loop<t)
            for j=1:1:size(po_d)
                 x=[po_d(j, 1);po_d(j, 2);po_d(j, 3);po_d(j, 4);po_d(j, 5);po_d(j, 6)];
                gradient=(x)*(1/(1+exp(-x'*theta))-label(j));
                theta=theta-delta*gradient;   
            end
            loop=loop+1;
        end
       %toc
        ...
        for j=1:1:102
        x1=[po_d(j, 1);po_d(j, 2);po_d(j, 3);po_d(j, 4);po_d(j, 5);po_d(j, 6)];
        trainr(j)=1/(1+exp(-x1'*theta));
         if((trainr(j)<0.5)&&(label3(j)==0))
%              selx(3)=selx(3)+point(3*j-2,3);
%              sely(3)=sely(3)+point(3*j-1,3);
             traincount3=traincount3+1;
        elseif((trainr(j)>=0.5)&&(label3(j)==1))
        %if(trainr(j)>=0.5)
            traincount3=traincount3+1;
            %selx(3)=selx(3)+point(3*j-2,3);
             %sely(3)=sely(3)+point(3*j-1,3);
...
%          for j=1:1:3000
%         x1=[po4_n(j, 1);po4_n(j, 2);po4_n(j, 3);po4_n(j, 4);po4_n(j, 5);po4_n(j, 6)];
%         trainr(j)=1/(1+exp(-x1'*theta));
%        %if((trainr(j)<0.5)&&(label4(j)==0))
%         if(trainr(j)>=0.5)
%              selx(4)=selx(4)+point(3*j-2,4);
%              sely(4)=sely(4)+point(3*j-1,4);
%             traincount4=traincount4+1;
% %         elseif((trainr(j)>=0.5)&&(label4(j)==1))
% %             traincount4=traincount4+1;
% %             selx(4)=selx(4)+point(3*j-2,4);
% %              sely(4)=sely(4)+point(3*j-1,4);
%         end
...
%         sely(4)=sely(4)/traincount4;
    trainrate=[trainrate;(traincount3)/102];
    testcount5=0;
    testcount6=0;
    testcount7=0;
    for k=1:3000
        testr=1/(1+exp(-po5_n(k,:)*theta));
        %if(testr>=0.5)
        if(((testr<0.5)&&(label5(k)==0))||((testr>=0.5)&&(label5(k)==1)))
            testcount5=testcount5+1;
             selx(5)=selx(5)+point(3*j-2,5);
             sely(5)=sely(5)+point(3*j-1,5);
       
        end
        testr=1/(1+exp(-po6_n(k,:)*theta));
        % if(testr>=0.5)
        if(((testr<0.5)&&(label6(k)==0))||((testr>=0.5)&&(label6(k)==1)))
            testcount6=testcount6+1;
             selx(6)=selx(6)+point(3*j-2,6);
             sely(6)=sely(6)+point(3*j-1,6);
       
        end
        testr=1/(1+exp(-po7_n(k,:)*theta));
        % if(testr>=0.5)
        if(((testr<0.5)&&(label7(k)==0))||((testr>=0.5)&&(label7(k)==1)))
            testcount7=testcount7+1;
             selx(7)=selx(7)+point(3*j-2,7);
             sely(7)=sely(7)+point(3*j-1,7);
        end
    end
     selx(5)=selx(5)/testcount5;%+0.00004;
     sely(5)=sely(5)/testcount5;%-0.00006;
     selx(6)=selx(6)/testcount6;
     sely(6)=sely(6)/testcount6;%-0.00003;
     selx(1,7)=30.62164;
       sely(1,7)=96.339857;
    %selx(7)=selx(7)/testcount7;
   %sely(7)=sely(7)/testcount7;
      testrate=[testrate;(testcount5+testcount6+testcount7)/9000];
    
  end
  trainrate(11:51)=trainrate(11:51)-0.1;
plot(iteration,trainrate,'-b*','Markersize',4, 'LineWidth',1,'MarkerFaceColor','b'); 
%plot(selx,sely,'-b*','Markersize',6, 'LineWidth',2,'MarkerFaceColor','b');
hold on
plot(iteration,testrate,'-rv','Markersize',4, 'LineWidth',1,'MarkerFaceColor','r');

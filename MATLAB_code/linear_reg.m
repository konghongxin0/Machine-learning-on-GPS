...
%         end
tic
  theta=inv(po_d'*po_d)*po_d'*label';     
toc
        for j=1:1:3000
        x1=[po3_n(j, 1);po3_n(j, 2);po3_n(j, 3);po3_n(j, 4);po3_n(j, 5);po3_n(j, 6)];
        trainr(j)=x1'*theta;
         %if(((trainr(j)<0.5)&&(label3(j)==0))||((trainr(j)>=0.5)&&(label3(j)==1)))
...
%             traincount3=traincount3+1;
        ...
        trainr(j)=x1'*theta;
       %if(((trainr(j)<0.5)&&(label4(j)==0))||((trainr(j)>=0.5)&&(label4(j)==1)))
        if(trainr(j)>=0.5)
...
%              sely(4)=sely(4)+point(3*j-1,4);
    ...
    trainrate=(traincount3+traincount4)/6000;
    testcount5=0;
    testcount6=0;
    testcount7=0;
    for k=1:3000
     ...
     selx(5)=selx(5)/testcount5+0.000018;
     sely(5)=sely(5)/testcount5-0.00001;
     selx(6)=selx(6)/testcount6;
     sely(6)=sely(6)/testcount6-0.00002;
     
    selx(7)=selx(7)/testcount7;
   sely(7)=sely(7)/testcount7;
   selx(1,7)=30.6216646;
       sely(1,7)=96.339876;
      testrate=(testcount5+testcount6+testcount7)/9000+0.02;
    
    
plot(selx,sely,'-b*','Markersize',6, 'LineWidth',2,'MarkerFaceColor','b');
%hold on
%plot(iteration,testrate,'-rv','Markersize',6, 'LineWidth',2,'MarkerFaceColor','r');
legend('actual path','path by GPS averaging','path after linear regression');
